...
# Always run download_model.sh, which handles both downloading and processing
# It's smart enough to use existing models if they exist
...
if [ -f "$SCRIPT_DIR/autogen.sh" ]; then
    ...
    # Run autogen.sh which will configure the build
